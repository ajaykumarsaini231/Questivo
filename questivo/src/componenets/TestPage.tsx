...
type Question = {
  ...
  /** Inline SVG figure, already sanitized server-side by lib/sanitizeSvg.js. */
  diagramSvg?: string | null;
  /** Raster figure from Google Drive, as a data: URI. Wins over diagramSvg. */
            ...
            <div className="mt-4 prose max-w-none">
              {loadingQuestion ? (
                ...
                <>
                  ...
                  {/* A Drive figure beats a generated one, so it renders in
                      preference. Only one is ever set server-side, but the
                      ordering makes the precedence explicit here too. */}
                  {question.diagramImage ? (
                    <figure className="mb-6 flex flex-col items-center">
                      <img
                        src={question.diagramImage}
                        alt={`Figure for question ${question.indexInSession}`}
                        className="max-h-72 w-auto max-w-full rounded-[10px] border border-slate-200 bg-white p-2"
                        loading="lazy"
                        decoding="async"
                      />
                    </figure>
                    ...
                    <figure className="mb-6 flex justify-center">
                      {/* The SVG is generated by the model, so it is sanitized on
                          the server (src/lib/sanitizeSvg.js) against an element
                          and attribute allow-list before it is ever stored.
                          Nothing here should be rendered that did not go through
                          that function. */}
                      <div
                        className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-3 [&>svg]:h-auto [&>svg]:w-full"
                        role="img"
                        aria-label={`Figure for question ${question.indexInSession}`}
